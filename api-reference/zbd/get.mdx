---
title: "Get ZBD Gamertag Payment"
...
The idea here is to use the `transactionId` property returned from the [Send Payment to ZBD Gamertags](/api-reference/zbd/send) response, and use that ID to fetch the latest details of the given Payment.
...
```bash Request
curl -L -X GET 'https://api.zebedee.io/v0/gamertag/transaction/:TRANSACTION_ID'
```
