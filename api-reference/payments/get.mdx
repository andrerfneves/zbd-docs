---
title: "Get Payment"
api: "GET https://api.zebedee.io/v0/payments/:PAYMENT_ID"
---

## Description

Retrieves all the information related to a specific Payment.

## Usage

Use this API endpoint to get all of the properties of a given Payment record. While the proper way is to use the `callbackUrl` property to receive Payment updates, you may use this API endpoint and fetch the Payment `status`, for example.

## Configuration

### Path Parameters

<ParamField required path="PAYMENT_ID" type="string">
  PAYMENT ID
</ParamField>

### Header Parameters

<ParamField required header="apikey" type="string">
  ZEBEDEE Project API Key
</ParamField>

<RequestExample>

```bash Request
curl -L -X GET 'https://api.zebedee.io/v0/payments/:PAYMENT_ID'
```

</RequestExample>

<ResponseExample>

```json Response
{
  "success": true,
  "message": "Fetched Payment.",
  "data": {
    "id": "caafd318-527c-466b-81f2-441d3a092aae",
    "fee": "2000",
    "unit": "msats",
    "amount": "101000",
    "invoice": "",
    "preimage": "8a14f6da89d4a8ffd09677f585b7c377de72744b7c3713d3c115fa71ca4fc290",
    "internalId": "11af01d092444a317cb33faa6b8304b8",
    "processedAt": "2023-01-04T15:48:29.805Z",
    "confirmedAt": "2023-01-04T15:48:29.805Z",
    "description": "Custom Payment Description",
    "status": "completed"
  }
}
```

</ResponseExample>
