...
In order to get data on payments sent to ZBD Usernames, you can use this endpoint. The data payload returned will inform you of the `status` of that transaction as well as any associated `fees`.

## Usage

The idea here is to use the `transactionId` property returned from the [Pay to ZBD Username](/api-reference/zbd-user/send) response, and use that ID to fetch the latest details of the given Payment.
