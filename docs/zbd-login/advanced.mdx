...
## Permissions and ZBD API[​](#permissions-and-zbd-api "Direct link to heading")
...
| --- | :-: | --- |
| ZBD Gamertag | `User` | The logged in user's ZBD Gamertag (e.g. `satoshi`) |
...
| Biography | `User` | The logged in user's biography description |
| Static Charge QR | `User` | The logged in user's unique static payment QR code (e.g. also seen at the user's personal ZBD.gg page [https://zbd.gg/zebedee](https://zbd.gg/zebedee)) |
| Verification Level | `User` | The logged in user's verification level with ZBD |
| Social URLs | `User` | The logged in user's social media/personal website URLs (configured manually by the user on the ZEBEDEE App) |
...
## Understanding Access Tokens[​](#understanding-access-tokens "Direct link to heading")
...
## Additional Security[​](#additional-security "Direct link to heading")
...
### PKCE[​](#pkce "Direct link to heading")
...
The OAuth 2.0 spec is the industry standard protocol for authorization and allows users to grant permission for apps to access their ZEBEDEE data. Using OAuth 2.0, users can access their ZEBEDEE data from third party applications without sensitive information (like passwords) ever changing hands.
...
If you find issues when setting `ZBD Login` or want to request new scopes or features, please reach us on [support@zebedee.io](mailto:support@zebedee.io).