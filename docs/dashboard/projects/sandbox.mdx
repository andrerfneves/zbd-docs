...
You can view all Sandbox APIs by heading over to the [Sandbox API Reference page](/api-reference/username/sandbox/send).

<Note>At the moment we have exposed the [**Send Bitcoin to ZBD Username Sandbox API**](/api-reference/username/sandbox/send) for testing purposes. We will be adding more Sandbox APIs in the future, including the ability to create Sandbox Charges and Static Charges, as well as send Sandbox Payments.
...
## Sending Bitcoin to a Sandbox ZBD Username
...
Now that we have our Sandbox API Key, let's use the [**Send Bitcoin to ZBD Username Sandbox API**](/api-reference/username/sandbox/send) to send some _fake satoshis_ to our Sandbox Test User.
...
<Frame type="glass" caption="Replace YOUR_API_KEY_HERE with your Sandbox API Key">
  <img src="/img/v2/devdash/sandbox-doc-portal2.png" alt="ZBD API Reference - Sandbox APIs" />
</Frame>
<br />

Head on over to the API Reference page and enter your Sandbox API Key and one of your Test User's ZBD Username. Then click on the `Send API Request` button on the top right.
...
You have just used the [**Send Bitcoin to ZBD Username Sandbox API**](/api-reference/username/sandbox/send) to send _fake satoshis_ to your Sandbox Test User.
